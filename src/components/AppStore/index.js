...
class AppStore extends Component {
  state = {activeid: appsList[0].category, searchelement: ''}

  getFiltereddata = () => {
    const {activeid} = this.state
    const filteredData = appsList.filter(
      eachList => eachList.category === activeid,
    )
    return filteredData
  }
  ...
  render() {
    const {activeid, searchelement} = this.state
    const filteredData = this.getFiltereddata()
    const searchFilteredData = filteredData.filter(item =>
      item.appName.toLowerCase().includes(searchelement.toLowerCase()),
    )
    return (
      <div className="container">
        ...
        <ul className="tabcontaienr">
          {tabsList.map(eachTab => (
            <TabItem
              eachTab={eachTab}
              key={eachTab.tabId}
              getcategory={this.getCategory}
              isequal={activeid === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="appContainer">
          {searchFilteredData.map(eachApp => (
